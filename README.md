...
3) Открыть консоль, перейти в папку с проектом и запустить оркестратор.
```
cd [Путь до проекта]/DistrArithmeticExpresCalc/cmd/orchestratorserv
go run main.go
```
4) Открыть вторую консоль, перейти в папку с проектом и запустить агента.
```
cd [Путь до проекта]/DistrArithmeticExpresCalc/cmd/agent
go run main.go
```
5) Открыть третью консоль, через которую добавить выражение на вычисление и получить результат.
```
